...
use crate::routes::restraint_report::{
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/restraint-report/pdf-stream", get(get_restraint_report_pdf_stream))
}
...
fn col_widths() -> Vec<f32> {
    vec![
        ...
        COL_DRIVE, COL_CARGO, COL_BREAK,
        COL_SUBTOTAL, COL_TOTAL, COL_CUMULATIVE,
        COL_DRIVE_AVG, COL_REST,
        COL_ACTUAL, COL_OVERTIME, COL_NIGHT, COL_OT_NIGHT,
    ]
}
...
async fn get_restraint_report_pdf(
    ...
    let drivers = if let Some(did) = filter.driver_id {
        sqlx::query_as::<_, Driver>(
            "SELECT id, tenant_id, driver_cd, driver_name FROM drivers WHERE tenant_id = $1 AND id = $2",
        )
        .bind(tenant_id)
        .bind(did)
        ...
        sqlx::query_as::<_, Driver>(
            "SELECT id, tenant_id, driver_cd, driver_name FROM drivers WHERE tenant_id = $1 ORDER BY driver_cd",
        )
        .bind(tenant_id)
        .fetch_all(&state.pool)
        .await
    ...
    if drivers.is_empty() {
        return Err((StatusCode::NOT_FOUND, "ドライバーが見つかりません".to_string()));
    }
    ...
    for driver in &drivers {
        ...
        let report = build_report_with_name(
            &state.pool, tenant_id, driver.id, &driver.driver_name,
            filter.year, filter.month,
        ).await?;
        driver_cds.push(driver.driver_cd.clone());
        reports.push(report);
    }

    let pdf_bytes = generate_pdf(&reports, &driver_cds, filter.year, filter.month).map_err(|e| {
        tracing::error!("PDF generation error: {e}");
        (StatusCode::INTERNAL_SERVER_ERROR, "PDF生成に失敗しました".to_string())
    })?;
...
async fn get_restraint_report_pdf_stream(
    ...
    tokio::spawn(async move {
        ...
        let drivers: Vec<_> = drivers.into_iter().filter(|d| !d.driver_name.is_empty()).collect();
        ...
        for (i, driver) in drivers.iter().enumerate() {
            send(PdfProgressEvent {
                event: "progress".into(), current: Some(i + 1), total: Some(total),
                driver_name: Some(driver.driver_name.clone()),
                step: Some("fetch".into()), data: None, message: None,
            }).await;

            match build_report_with_name(
                &state.pool, tenant_id, driver.id, &driver.driver_name, year, month,
            ).await {
                Ok(report) => {
                    driver_cds.push(driver.driver_cd.clone());
                    reports.push(report);
        ...
        send(PdfProgressEvent {
            event: "progress".into(), current: Some(total), total: Some(total),
            driver_name: None, step: Some("render".into()),
            data: None, message: Some("PDF生成中...".into()),
        }).await;
        ...
        match generate_pdf(&reports, &driver_cds, year, month) {
            Ok(pdf_bytes) => {
                ...
                send(PdfProgressEvent {
                    event: "done".into(), current: Some(total), total: Some(total),
                    driver_name: None, step: Some("save".into()),
                    data: Some(b64), message: None,
                }).await;
            }
            Err(e) => {
                send(PdfProgressEvent {
...
fn render_driver_page(
    ...
    add_text(&mut ops, doc, font_id, MARGIN_LEFT, y, FONT_SIZE_TITLE, "拘 束 時 間 管 理 表");
    add_text(&mut ops, doc, font_id, MARGIN_LEFT + 68.0, y, FONT_SIZE_HEADER, &format!("{}年 {}月分", year, month));
    y -= 5.0;

    let name_label = format!("氏 名　( {} ) {}", driver_cd, report.driver_name);
    add_text(&mut ops, doc, font_id, MARGIN_LEFT, y, FONT_SIZE_HEADER, &name_label);
    y -= 4.0;

    let max_label = format!("当月の最大拘束時間: {}時間", report.max_restraint_minutes / 60);
    add_text_right(&mut ops, doc, font_id, PAGE_W - MARGIN_RIGHT, y, FONT_SIZE_SMALL, &max_label);
    y -= 4.0;
    ...
    for (text, w) in &groups {
        ...
        add_text_center_in_cell(&mut ops, doc, font_id, x, *w, header_y - 1.8, FONT_SIZE_SMALL, line1);
        // Second line if exists
        if let Some(line2) = text.lines().nth(1) {
            add_text_center_in_cell(&mut ops, doc, font_id, x, *w, header_y - HEADER_ROW_H - 0.5, FONT_SIZE_SMALL, line2);
        }
        x += w;
    }
    draw_vline(&mut ops, x, header_y, header_y - HEADER_ROW_H * 2.0, LINE_THIN);
    ...
    for (text, w) in &row2 {
        ...
        if !text.is_empty() {
            add_text_center_in_cell(&mut ops, doc, font_id, x, *w, y - 1.5, FONT_SIZE_SMALL, text);
        }
        x += w;
    }
    draw_vline(&mut ops, x, y, y - HEADER_ROW_H, LINE_THIN);
    ...
    let reiwa_fy = if month >= 4 { reiwa_year } else { reiwa_year - 1 };
    let fc = if fiscal_cum.is_empty() { "0:00" } else { &fiscal_cum };
    let yt = if year_total.is_empty() { "0:00" } else { &year_total };

    add_text(&mut ops, doc, font_id, MARGIN_LEFT, y, FONT_SIZE_SMALL,
        &format!("4月〜前月 累計拘束時間　　{}",  fc));
    let mt = if month_total_str.is_empty() { "0:00" } else { &month_total_str };
    add_text(&mut ops, doc, font_id, MARGIN_LEFT + 80.0, y, FONT_SIZE_SMALL,
        &format!("{}", mt));
    y -= 3.5;
    add_text(&mut ops, doc, font_id, MARGIN_LEFT, y, FONT_SIZE_SMALL,
        &format!("{}年度　拘束時間　　{} 時間", reiwa_fy + 2018, yt));

    add_text_right(&mut ops, doc, font_id, PAGE_W - MARGIN_RIGHT, y, FONT_SIZE_SMALL,
        "大石運輸倉庫　株式会社");

    y -= 4.0;
    add_text(&mut ops, doc, font_id, PAGE_W - MARGIN_RIGHT - 90.0, y, FONT_SIZE_SMALL,
        "* : 2マン運行");
    y -= 3.0;
    add_text(&mut ops, doc, font_id, PAGE_W - MARGIN_RIGHT - 90.0, y, FONT_SIZE_SMALL,
        "D2 : 2分割休息　D3 : 3分割休息");
...
fn draw_day_row_holiday(
    ...
    add_text_center_in_cell(ops, doc, font_id, cx, all_widths[0], y - 1.5, FONT_SIZE_BODY, &date_str);
    cx += all_widths[0];

    // "休" in start column
    draw_vline(ops, cx, y, y - h, LINE_THIN);
    add_text_center_color(ops, doc, font_id, cx, all_widths[1], y - 1.5, FONT_SIZE_BODY, "休", 0.8, 0.2, 0.2);
    cx += all_widths[1];
...
fn draw_day_row(
    ...
    // 3: 運転（主運行）
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[3], main_y, fs,
        &fmt_minutes(day.drive_minutes));
    if has_overlap && day.overlap_drive_minutes > 0 {
        add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[3], overlap_y, FONT_SIZE_OVERLAP,
    ...
    // 4: 荷役
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[4], main_y, fs,
        &fmt_minutes(day.cargo_minutes));
    if has_overlap && day.overlap_cargo_minutes > 0 {
        add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[4], overlap_y, FONT_SIZE_OVERLAP,
    ...
    // 5: 休憩
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[5], main_y, fs,
        &fmt_minutes(day.break_minutes));
    if has_overlap && day.overlap_break_minutes > 0 {
        add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[5], overlap_y, FONT_SIZE_OVERLAP,
    ...
    // 6: 小計（主運行の拘束小計）
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[6], main_y, fs,
        &fmt_minutes(day.restraint_main_minutes));
    if has_overlap {
        add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[6], overlap_y, FONT_SIZE_OVERLAP,
    ...
    // 7: 合計
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[7], main_y, fs,
        &fmt_minutes(day.restraint_total_minutes));
    x += all_widths[7];

    // 8: 拘束累計
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[8], main_y, fs,
    ...
    if let Some(rest) = day.rest_period_minutes {
        add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[10], main_y, fs,
            &fmt_minutes(rest));
    }
    x += all_widths[10];
    ...
    // 11: 実働
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[11], main_y, fs,
        &fmt_minutes(day.actual_work_minutes));
    x += all_widths[11];

    // 12: 時間外
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[12], main_y, fs,
        &fmt_minutes(day.overtime_minutes));
    x += all_widths[12];

    // 13: 深夜
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[13], main_y, fs,
        &fmt_minutes(day.late_night_minutes));
    x += all_widths[13];

    // 14: 時間外深夜
    draw_vline(ops, x, y, y - row_h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[14], main_y, fs,
    ...
    if !day.remarks.is_empty() {
        add_text(ops, doc, font_id, x + 0.5, main_y, FONT_SIZE_SMALL, &day.remarks);
    }
    x += all_widths[15];
...
fn draw_weekly_subtotal(
    ...
    for (i, w) in all_widths.iter().enumerate() {
        ...
        match i {
            3 => add_text_right_in_cell(ops, doc, font_id, x, x + w, ty, fs, &fmt_minutes(ws.drive_minutes)),
            4 => add_text_right_in_cell(ops, doc, font_id, x, x + w, ty, fs, &fmt_minutes(ws.cargo_minutes)),
            5 => add_text_right_in_cell(ops, doc, font_id, x, x + w, ty, fs, &fmt_minutes(ws.break_minutes)),
            6 => add_text_right_in_cell(ops, doc, font_id, x, x + w, ty, fs, &fmt_minutes(ws.restraint_minutes)),
            _ => {}
        }
        x += w;
    }
...
fn draw_monthly_total(
    ...
    // 11: 実働合計
    draw_vline(ops, x, y, y - h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[11], ty, fs, &fmt_minutes(total.actual_work_minutes));
    x += all_widths[11];

    // 12: 時間外合計
    draw_vline(ops, x, y, y - h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[12], ty, fs, &fmt_minutes(total.overtime_minutes));
    x += all_widths[12];

    // 13: 深夜合計
    draw_vline(ops, x, y, y - h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[13], ty, fs, &fmt_minutes(total.late_night_minutes));
    x += all_widths[13];

    // 14: 時間外深夜合計
    draw_vline(ops, x, y, y - h, LINE_THIN);
    add_text_right_in_cell(ops, doc, font_id, x, x + all_widths[14], ty, fs, &fmt_minutes(total.overtime_late_night_minutes));
    x += all_widths[14];
...
fn lp(x_mm: f32, y_mm: f32) -> LinePoint {
    LinePoint { p: Point::new(Mm(x_mm), Mm(y_mm)), bezier: false }
}

fn draw_hline(ops: &mut Vec<Op>, x1: f32, x2: f32, y: f32, _thickness: f32) {
    ops.push(Op::SaveGraphicsState);
    ops.push(Op::SetOutlineColor {
        col: Color::Rgb(Rgb { r: 0.3, g: 0.3, b: 0.3, icc_profile: None }),
    });
    ops.push(Op::DrawLine {
        line: Line { points: vec![lp(x1, y), lp(x2, y)], is_closed: false },
    });
    ops.push(Op::RestoreGraphicsState);
}
...
fn draw_vline(ops: &mut Vec<Op>, x: f32, y1: f32, y2: f32, _thickness: f32) {
    ...
    ops.push(Op::DrawLine {
        line: Line { points: vec![lp(x, y1), lp(x, y2)], is_closed: false },
    });
    ops.push(Op::RestoreGraphicsState);
}
