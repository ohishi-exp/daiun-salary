---
name: daiun-salary-map
generated-from: daiun-salary:cd0754ee81c98ebced561c644411b022655522ba
paths: [src/, migrations/]
description: 北海大運（Hokkaido Daiun）給与管理システム (Rust/Axum) の構造ナビゲーション。domain/engine/compare/repository/api/import の区画、compare/mod.rs の関数構造、web地球号（デジタコ）設定値・拘束時間計算式・金額丸め規約・Excel互換・R2ストレージ構造・Known Bugs をまとめる。トリガー:「daiun-salary」「給与計算」「拘束時間管理表」「compare/mod.rs」「web地球号」「KUDGIVT」「KUDGURI」「KUDGFRY」「フェリー控除」「Known Bugs」「時間外計算」「深夜時間」「休息期間」「変形労働時間制」等。
---

# daiun-salary-map

北海大運の給与管理システム（Excel → Rust/Axum 移行）の構造 map。詳細は CLAUDE.md
本体でなくこの skill を参照する（lazy load）。

## 区画

| 区画 | 主要ファイル | 役割 |
|---|---|---|
| domain | `src/domain/` | ドメインモデル（Employee, Attendance, PayrollPeriod 等） |
| engine | `src/engine/` | 給与計算エンジン（time_calc / overtime / holiday / allowance / summary / payroll） |
| compare | `src/compare/mod.rs` | 拘束時間管理表 CSV 比較（コア計算モジュール、~3600行） |
| repository | `src/repository/` | DB アクセス層（SQLx CRUD） |
| api | `src/api/` | Axum ハンドラ・ルーティング・DTO |
| import | `src/import/` | Excel インポート（calamine） |

## entrypoint

- `src/main.rs` — Axum サーバーエントリポイント

## 関連

- 設計書: `docs/給与管理_Rust実装計画.md`
- フロントエンド: `/home/yhonda/js/nuxt-dtako-admin`
...
- 月締め時刻: 24:00
- 丸め単位時間: 1分
- **秒数は使わない**: web地球号の集計は全て分単位。時刻比較・gap計算・duration集計は `trunc_min`（秒切り捨て）後の分単位で行う。秒ベースの四捨五入 `(secs+30)/60` は丸め誤差の原因になるため避ける
